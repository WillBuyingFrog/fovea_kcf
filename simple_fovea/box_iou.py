...
def box_iou_numpy(box1, box2):
    ...
    Args:
        box1 (numpy.ndarray): 第一个边界框，形状为(N, 4)，其中N是框的数量。
        box2 (numpy.ndarray): 第二个边界框，形状为(M, 4)，其中M是框的数量。

    Returns:
        numpy.ndarray: 形状为(N, M)的IoU矩阵，其中每个元素是box1中的一个框与box2中的一个框的IoU。
    """
    # 计算交集的坐标
    ...
    # 计算并集的面积
    box1_area = ((box1[:, 2] - box1[:, 0]) * (box1[:, 3] - box1[:, 1])).reshape(-1, 1)
    box2_area = ((box2[:, 2] - box2[:, 0]) * (box2[:, 3] - box2[:, 1])).reshape(1, -1)
    union_area = box1_area + box2_area - inter_area
